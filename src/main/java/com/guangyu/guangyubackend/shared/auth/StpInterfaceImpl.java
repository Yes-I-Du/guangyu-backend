...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        if (loginUser == null) {
            throw new BusinessException(RespCode.NO_AUTH_ERROR, "用户未登录");
        }
        ...
        if (spaceUserId != null) {
            ...
            if (spaceUser == null) {
                throw new BusinessException(RespCode.NOT_FOUND_ERROR, "未找到空间用户信息");
            }
            // 取出当前登录用户对应的 spaceUser
            SpaceUser loginSpaceUser =
                spaceUserRepository.lambdaQuery().eq(SpaceUser::getSpaceId, spaceUser.getSpaceId())
        ...
        if (spaceId == null) {
            ...
            Picture picture = pictureRepository.lambdaQuery().eq(Picture::getId, pictureId)
                .select(Picture::getId, Picture::getSpaceId, Picture::getUserId).one();
            if (picture == null) {
                throw new BusinessException(RespCode.NOT_FOUND_ERROR, "未找到图片信息");
            }
        ...
        if (space == null) {
            throw new BusinessException(RespCode.NOT_FOUND_ERROR, "未找到空间信息");
        }
        // 根据 Space 类型判断权限
        if (space.getSpaceType() == SpaceTypeEnum.PRIVATE.getValue()) {
            ...
            spaceUser =
                spaceUserRepository.lambdaQuery().eq(SpaceUser::getSpaceId, spaceId).eq(SpaceUser::getUserId, userId)
                    .one();
            if (spaceUser == null) {
                return new ArrayList<>();
            }
